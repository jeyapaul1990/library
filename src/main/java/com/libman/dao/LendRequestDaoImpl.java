...
public class LendRequestDaoImpl implements LendRequestDao{
    ...
    public List<Lends> getLentRecords() {
		return jdbcTemplate.query("SELECT * FROM lendrequest where status='Requested' ", new RowMapper<Lends>() {
	...
	public int lendRequestApproveAction(Lends lend) {
		...
		String approvequery = "Update lendrequest set status = ? , defaultreturndate = ?  where lendid = " +lend.getLendid()+"";
	...
	public int lendRequestRejectAction(Lends lend) {
		...
		String rejectquery = "Update lendrequest set status = ? , defaultreturndate = ?, actualreturndate = ? where lendid = " +lend.getLendid()+"";
	...
	public List<Lends> getApprovedLentRecords() {
		return jdbcTemplate.query("SELECT * FROM lendrequest where status='Approved' ", new RowMapper<Lends>() {
			...
			public Lends mapRow(ResultSet rs, int rowNum) throws SQLException {
				...
				lend.setRequeststatus(rs.getString("status"));
				return lend;
			}
		});
	}
	...
	public List<Lends> getLentRecords(int userid) {
		...
		return jdbcTemplate.query("SELECT * FROM lendrequest where userid = "+userid+" and status= 'Approved' ", new RowMapper<Lends>() {
